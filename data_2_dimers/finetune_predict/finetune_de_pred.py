...
# SMILES randomization for data augmentation, n number of non-canonical SMIs
...
# Model with attention pooling + dropout
class ChemBERTaForRegression(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        pooled = torch.sum(hidden_states * attn_weights.unsqueeze(-1), dim=1)  # (batch, hidden_dim)
        pooled = self.dropout(pooled)
        output = self.regressor(pooled).squeeze(-1)
        return output

# Load data
data = pd.read_csv('./labeled_data_de.csv')
smiles = data['SMILES'].tolist()
labels = data['dE'].to_numpy()  

# Train/val split
smiles_train, smiles_val, labels_train, labels_val = train_test_split(smiles, labels, test_size=0.1, random_state=42)

# Tokenizer
pretrained_dir = os.path.expanduser("PATH_TO/chemberta_380k/")
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), "chemberta_finetuned_de_state.pt")
        print(f"Best model saved at epoch {epoch+1} with val_loss = {val_loss:.4f}")
pd.DataFrame(loss_log).to_csv("loss_log_de.csv", index=False)
...
df.to_csv("de_predictions.csv", index=False)
print("Predictions saved to de_predictions.csv")
